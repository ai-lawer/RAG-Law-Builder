...
def txt_to_csv(folder_path, isAr):
    ...
    for file_name in os.listdir(folder_path):
        if isAr:
            if (
                ...
                and not file_name.startswith("Federal")
                and not file_name.startswith("Cabinet")
            ):
                file_path = os.path.join(folder_path, file_name)

                content = get_txt_content(file_path)

                indices = get_arabic_matches_indices(content)
...
def get_arabic_matches_indices(content):
    previous_number = 0
    indices = []
    matches = re.finditer(
        r"املادة \(\s*(\d+)\s*\)",
